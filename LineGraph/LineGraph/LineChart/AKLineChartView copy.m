...
- (id)initWithFrame:(CGRect)frame
{
	...
	if (self) {
		...
		_xLabelFont = [UIFont fontWithName:@"Avenir-Medium" size:10];
...
- (void)drawRect:(CGRect)rect
{
	...
	for (NSUInteger i=0; i<[self.xLabels count]; i++)
    {
		...
		CGContextAddLineToPoint(ctx, x, self.frame.size.height-top_margin+bottom_margin);
		CGContextStrokePath(ctx);
	}
    ...
    [[UIColor colorWithRed:0.089 green:0.395 blue:0.824 alpha:.2] setFill];
